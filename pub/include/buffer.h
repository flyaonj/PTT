...
#ifndef _MCPTT_BUFFER_H
#define _MCPTT_BUFFER_H
...
public:
    ...
    explicit Buffer(size_t initial_size = kInitialSize, size_t reserved_prepend_size = kCheapPrependSize,bool byteOrderSwap = false)
        ...
        , byteOrderSwap(byteOrderSwap){
        buffer_ = new char[capacity_];
        poco_assert(length() == 0);
        poco_assert(WritableBytes() == initial_size);
        poco_assert(PrependableBytes() == reserved_prepend_size);
    }
...
public:
    void Write(const void* /*restrict*/ d, size_t len) {
        ...
        poco_assert(write_index_ + len <= capacity_);
        write_index_ += len;
    }
    ...
    void Prepend(const void* /*restrict*/ d, size_t len) {
        poco_assert(len <= PrependableBytes());
        read_index_ -= len;
...
public:
    ...
    void ReadBytes(void* d, size_t len)
    {
        poco_assert(length() >= len);
        ::memcpy(d, data(), len);
        Skip(len);
    }
...
public:
    ...
    int64_t PeekInt64() const {
        poco_assert(length() >= sizeof(int64_t));
        int64_t be64 = 0;
    ...
    int32_t PeekInt32() const {
        poco_assert(length() >= sizeof(int32_t));
        int32_t be32 = 0;
    ...
    int16_t PeekInt16() const {
        poco_assert(length() >= sizeof(int16_t));
        int16_t be16 = 0;
    ...
    int8_t PeekInt8() const {
        poco_assert(length() >= sizeof(int8_t));
        int8_t x = *data();
        return x;
    }
...
public:
    ...
    size_t length() const {
        poco_assert(write_index_ >= read_index_);
    ...
    size_t WritableBytes() const {
        poco_assert(capacity_ >= write_index_);
...
public:
    ...
    const char* FindCRLF(const char* start) const {
        poco_assert(data() <= start);
        poco_assert(start <= WriteBegin());
